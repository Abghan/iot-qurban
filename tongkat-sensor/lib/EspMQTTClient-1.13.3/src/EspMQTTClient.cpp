...
void EspMQTTClient::loop()
{
  ...
  if(wifiStateChanged) return;

  if(enable_MQTT)
  {
  // MQTT Handling
  bool mqttStateChanged = handleMQTT();
  if(mqttStateChanged) return;
...
uint16_t EspMQTTClient::getMaxPacketSize()
{
  uint16_t my_size = _mqttClient.getBufferSize();
  return my_size;
}
...
bool EspMQTTClient::publish(const String &topic, const String &payload, bool retain)
{
  ...
  Serial.println(_mqttClient.size_payload());
...
void EspMQTTClient::connectToWifi()
{
  ...
  if (_enableSerialLogs)
    Serial.printf("\nWiFi: Connecting to %s  %s... (%fs) \n", _wifiSsid, _wifiPassword, millis()/1000.0);
}
...
bool EspMQTTClient::connectToMqttBroker()
{
  ...
  if (_mqttServerIp != nullptr && strlen(_mqttServerIp) > 0)
  {
    ...
    {
      if (_mqttUsername)
        Serial.printf("MQTT: Connecting to broker \"%s\" with client name \"%s\" and username \"%s\" and Pass \"%s\"... (%fs)", _mqttServerIp, _mqttClientName, _mqttUsername, _mqttPassword,millis()/1000.0);
      else
        Serial.printf("MQTT: Connecting to broker \"%s\" and Port \"%d\" with client name \"%s\" ... (%fs)", _mqttServerIp, _mqttServerPort, _mqttClientName, millis()/1000.0);
    }
    ...
    if (_enableSerialLogs) Serial.printf("Server IP: \"%s\" & Port: \"%d\"", _mqttServerIp, _mqttServerPort);
