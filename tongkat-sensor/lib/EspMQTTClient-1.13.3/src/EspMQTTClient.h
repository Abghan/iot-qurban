...
public:
  ...
  inline void setMqttServer(const char* ssid, const char* pass, const char* server, const char* username = "", const char* password = "", const short port = 1883) { // Allow setting the MQTT info manually (must be done in setup())
    _wifiSsid = ssid;
    _wifiPassword = pass;
